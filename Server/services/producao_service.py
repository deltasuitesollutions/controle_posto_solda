...
def _buscar_operacao_id(operacao_codigo: str) -> Optional[int]:
    """Busca ID da operação pelo código"""
    from Server.services import operacao_service
    operacoes = operacao_service.listar_operacoes()
    operacao = next((op for op in operacoes if op.get('operacao') == operacao_codigo), None)
...
def registrar_entrada(
    ...
    operacao_id = _buscar_operacao_id(operacao) if operacao else None
