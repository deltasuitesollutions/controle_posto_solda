...
def obter_hostname_raspberry() -> str:
    """
    Obtém o hostname do Raspberry Pi
    """
    try:
        # Método 1: socket.gethostname() (mais confiável)
        ...
        print(f"Erro ao obter hostname do Raspberry: {e}")
        return 'ERROR'
...
def registrar_dispositivo_raspberry() -> Dict[str, Any]:
    ...
    Obtém serial e hostname e salva no banco se ainda não existir
    """
    try:
        ...
        if dispositivo_existente:
            # Dispositivo já registrado, apenas atualizar hostname se mudou
            hostname_atual = obter_hostname_raspberry()
            if hostname_atual != 'UNKNOWN' and hostname_atual != dispositivo_existente.get('hostname'):
                dispositivo = DispositivoRaspberry.buscar_por_id(dispositivo_existente.get('id'))
                if dispositivo:
                    dispositivo.hostname = hostname_atual
                    dispositivo.save()
        ...
        serial = obter_serial_raspberry()
        hostname = obter_hostname_raspberry()
        
        if serial == 'UNKNOWN' or serial == 'TIMEOUT' or serial == 'ERROR':
            raise Exception("Não foi possível obter serial do dispositivo")
        
        if hostname == 'UNKNOWN' or hostname == 'TIMEOUT' or hostname == 'ERROR':
            raise Exception("Não foi possível obter hostname do dispositivo")
        
        # Criar novo dispositivo (apenas uma vez)
        novo_dispositivo = DispositivoRaspberry.criar(serial=serial, hostname=hostname)
        print(f"Dispositivo Raspberry registrado pela primeira vez: Serial={serial}, Hostname={hostname}")
