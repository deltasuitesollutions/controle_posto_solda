...
// CHAMADA PARA DISPOSITIVO_RASPBERRY_CONTROLLER.PY

export const dispositivosRaspberryAPI = {
  listar: () => fetchAPI('/dispositivos-raspberry'),
  buscarPorId: (id: number) => fetchAPI(`/dispositivos-raspberry/${id}`),
  buscarPorSerial: (serial: string) => fetchAPI(`/dispositivos-raspberry/por-serial/${serial}`),
  registrar: () => fetchAPI('/dispositivos-raspberry/registrar', { method: 'POST' }),
  criar: (serial: string, nome?: string) =>
    fetchAPI('/dispositivos-raspberry', {
      method: 'POST',
      body: JSON.stringify({ serial, nome }),
    }),
  atualizarNome: (id: number, nome: string) =>
    fetchAPI(`/dispositivos-raspberry/${id}/nome`, {
      method: 'PUT',
      body: JSON.stringify({ nome }),
    }),
  remover: (id: number) =>
    fetchAPI(`/dispositivos-raspberry/${id}`, { method: 'DELETE' }),
}
...
export const ihmAPI = {
  ...
  buscarContextoOperacao: (funcionario?: string) => 
    fetchAPI(`/ihm/contexto-operacao${funcionario ? `?funcionario=${encodeURIComponent(funcionario)}` : ''}`),
