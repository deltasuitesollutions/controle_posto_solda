...
const Funcionarios = () => {
    ...
    const carregarFuncionarios = async () => {
        ...
        try {
            ...
            setTituloErro('Erro!')
            setMensagemErro(`Erro ao carregar funcionários: ${error?.message || 'Erro desconhecido'}`)
    ...
    const handleConfirmarExclusao = async () => {
        ...
        try {
            ...
            setTituloErro('Erro!')
            setMensagemErro(`Erro ao excluir funcionário: ${errorMessage}`)
            setModalErroAberto(true)
        }
    }
                                                ...
                                                {operacoesDropdownAberto && (
                                                    <div className="absolute z-50 w-full top-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-y-auto">
                                                        {operacoesDisponiveis.length > 0 ? (
                                                            ...
                                                            <div className="px-3 py-2 text-sm text-gray-500">Carregando operações...</div>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                                                        ...
                                                        {funcionariosPaginaAtual.map((funcionario) => {
                                                            ...
                                                            <tr key={funcionarioId} className="hover:bg-gray-50">
                                                                ...
                                                                <td className="px-6 py-4 text-sm text-gray-900">
                                                                    {funcionario.operacoes_habilitadas && funcionario.operacoes_habilitadas.filter(op => op.habilitada !== false).length > 0 ? (
                                                                        <div className="flex flex-wrap gap-1">
                                                                            {funcionario.operacoes_habilitadas.filter(op => op.habilitada !== false).map((op) => (
                                                                                <span 
                                                                                    key={op.operacao_habilitada_id || op.operacao_id}
